...
function buildNewsUrl(assetType, symbol, timeframe, apiToken) {
  ...
  switch (timeframe) {
    case "1D":
      publishedAfter = new Date(now.getTime() - 3 * 24 * 60 * 60 * 1e3);
      break;
    case "1W":
      publishedAfter = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1e3);
      break;
    case "1M":
      publishedAfter = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1e3);
      break;
    default:
      publishedAfter = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1e3);
  }
...
*Note: This is a MOCK response. The GEMINI_API_KEY is either not configured or there was an error calling the API. Check the Worker logs for details.*`;
  ...
  const priceChange = hasData && chartData.length > 1 ? ((chartData[chartData.length - 1].close - chartData[0].close) / chartData[0].close * 100).toFixed(2) : null;
